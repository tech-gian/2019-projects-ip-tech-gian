...
void solve(int n, int m, int **board) {
    int i, j, temp_sum, ms, maxj=-1;
    int **sums;

    // Δέσμευση μνήμης για τον πίνακα μερικών αθροισμάτων
    ...
    // Προσπέλαση όλων των στοιχείων του πίνακα αυτού και συμπλήρωση του
    for (i=0 ; i<n ; i++) {
        for (j=0 ; j<m ; j++) {
            sumdp(i, j, board, sums, m);
        }
    }

    // Προσπέλαση της τελευταίας γραμμής του πίνακα μερικών αθροισμάτων
    i = n-1;
    for (j=0 ; j<m ; j++) {
        temp_sum = sums[i][j];
    ...
    // Αποδέσμευση μνήμης του πίνακα των αθροισμάτων
    for (i=0 ; i<n ; i++) {
	    free(*(sums+i));
    }
    free(sums);
